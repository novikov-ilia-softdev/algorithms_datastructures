...
module.exports.show = function( script, settings){
	...
	script.getRemoteObjectHelper().getAttr( settings.sdrRemoteObject, script.attributes.sdrFhssSyncPeriod.name, function (err, res) {
		if( !err){
			var lineEdit = script.widgetManager.createLineEdit( settings.title, res);

			lineEdit.getInput(function (value) {
				script.getRemoteObjectHelper().setAttr( settings.sdrRemoteObject, script.attributes.sdrFhssSyncPeriod.name, value, function (err, res) {
					err ?
